...
1. **Autoload 注册顺序** *(silent-bug 防御)*：`LifecycleService` 必须是 `[autoload]` 列表中**最后一个 Foundation 服务**。完整顺序：`PersistenceService` (per Persistence Core Rule 10) → `InputService` (per Input Core Rule 1) → `AudioSystem` → `LifecycleService`。Lifecycle 在自己的 `_ready()` 中读取其他 Foundation 状态时，必须使用 `call_deferred` 或 `tree_entered` 信号（遵循 Persistence Core Rule 10 的对称约束）。

2. **App Ready 判定**：`LifecycleService._ready()` 完成时，通过 `call_deferred` 触发 ready 验证：检查 `PersistenceService` 处于 `READY_*` 状态、`InputService` 处于 `GESTURE_MODE`、`AudioSystem.is_ready() == true`。三者全部就绪 → 发出 `app_ready` 信号。**`app_ready` 在整个 App 生命周期内只发出一次**。
...
3. **OS 通知订阅范围（明确边界）**：Lifecycle **仅订阅以下两个 OS 通知**，用于驱动自己的状态机：
   ...
   Lifecycle **不为三大 Foundation 同侪转发任何 OS 通知**——Persistence、Input、Audio 各自直接订阅，Lifecycle 不做中介。
...
| `BOOTING` | `_ready()` 入口 | Foundation 三同侪全部就绪检测通过 | 在 `call_deferred` 中轮询 Persistence/Input/Audio 状态 |
...
| `BOOTING` | 三 Foundation 全部就绪 | `READY` | 发出 `app_ready` |
...
|------|------|---------------|-----------------------|
| **PersistenceService** | Lifecycle → Persistence | 无（Persistence 不订阅 Lifecycle 信号） | `is_ready` 状态查询（验证 `READY_*`），仅在 `BOOTING` 阶段使用 |
| **InputService** | Lifecycle → Input | 无（Input 不订阅 Lifecycle 信号） | `InputService.release_text_mode()`（在 PAUSED_CONFIRMED → READY 转换时调用，IME 中断恢复策略，Core Rule 6） |
| **AudioSystem** | Lifecycle → Audio | 无（Audio 直接订阅 OS 通知处理自己的 shred_loop） | `is_ready` 状态查询（验证 BGM 已启动），仅在 `BOOTING` 阶段使用 |
...
```
boot_ready(now) = persistence_ready AND input_ready AND audio_ready

其中：
  persistence_ready = PersistenceService.state ∈ { READY_FRESH, READY_LOADED, READY_CORRUPTED }
  input_ready       = InputService.current_mode() == InputMode.GESTURE_MODE
  audio_ready       = AudioSystem.is_ready() == true  (BGM 启动 + 池初始化)
```

| 变量 | 类型 | 范围 | 说明 |
|------|------|------|------|
| `persistence_ready` | bool | true/false | Persistence 从 `UNINITIALIZED` 转出后即为 true（即使是 `READY_CORRUPTED`）|
| `input_ready` | bool | true/false | InputService `_ready()` 完成后默认是 `GESTURE_MODE` |
| `audio_ready` | bool | true/false | 由 AudioSystem 在 `_ready()` 完成且 BGM 启动后置为 true |
...
**示例**：
- 正常路径：Lifecycle `_ready()` 末尾 deferred → 下一帧 `_check_boot_ready` 评估三个 `_ready` 都为 true → `app_ready` 发出 → 状态 `BOOTING` → `READY`。耗时通常 < 16ms（单帧内）。
- 异常路径：Audio BGM 资源缺失 → AudioSystem 打印错误但不 ready → `audio_ready = false` → Lifecycle 持续 deferred 轮询 → `BOOT_TIMEOUT_MS` 触发警告。
...
- **若 AudioSystem BGM 资源缺失（Audio Edge Case "如果 `BGM_main_loop` 资源为 null"）**：AudioSystem 打印 error 但不进入 ready 状态 → `audio_ready = false` 永远不为 true → Lifecycle 永远停在 `BOOTING`。`BOOT_TIMEOUT_MS = 5000` 触发 → 发出 `boot_timeout` 诊断信号，`push_error("Lifecycle stuck in BOOTING: audio_ready=false")`。**这是显性失败**，开发者能在 console / CI 中第一时间看到，比"假装 ready 然后无 BGM"安全得多。
...
|---|---|---|---|
| `PersistenceService` | 读 `.state ∈ READY_*` | 只读状态查询 | Formula 1 boot ready 谓词 |
| `InputService` | 读 `current_mode()`；调用 `release_text_mode()` | 只读 + 一处写 | Formula 1 谓词 + Core Rule 6 IME 恢复 |
| `AudioSystem` | 读 `is_ready()` | 只读状态查询 | Formula 1 谓词 |

> ⚠️ 这三处访问不构成传统意义的"上游依赖"（Lifecycle 不订阅它们的信号、不依赖它们的功能契约），但**它要求 Foundation 三同侪满足以下接口约定**：
> - PersistenceService 暴露 `state` 字段或等价查询（per Persistence Core Rule 9 中的 `State` 列）
> - InputService 暴露 `current_mode()`（已在 Input API 中）和 `release_text_mode()`（已在 Input API 中）
> - AudioSystem 暴露 `is_ready()` —— **当前 Audio GDD 中无此方法**，需要回填（见 Bidirectional Consistency）
...
| **Haptic System** *(future, #4)* | 与 Lifecycle 无直接关系。Haptic 若需要响应"App 进入后台时停止振动"，应自行订阅 OS 通知（与三 Foundation 同侪同模式），不依赖 Lifecycle 转发。 |
| **Juice Cookbook** *(future, Wave 2)* | 无关系。Juice 不订阅生命周期信号——它通过 Lever、Shred、Silhouette 的游戏内信号触发。 |

### Bidirectional Consistency — 待处理项

1. **AudioSystem `is_ready()` 方法不存在**：Audio GDD 当前没有 `is_ready()` 公开方法。Lifecycle 依赖此 API → **需要在 Audio GDD 的 `/consistency-check` 中回填**，建议添加：
   ```gdscript
   ## AudioSystem._ready() 完成且 BGM 启动后返回 true。
   func is_ready() -> bool
   ```
   作为 Audio Core Rule 6（BGM 管理）的实现细节暴露。

2. **PersistenceService `state` 字段暴露**：Persistence GDD 描述了 5 个状态但 API 接口中未明确暴露 `state` 查询。需要 Persistence 添加：
   ```gdscript
   ## 当前 Persistence 状态。
   func get_state() -> State
   ```
   或类似只读访问器。**需通过 `/consistency-check` 回填**。
...
- **是否为 Foundation 三同侪转发 OS 通知**—— **否**。这是架构边界，不是开关。如果未来发现 Foundation 同侪应当通过 Lifecycle 路由，应当通过 ADR 评估而非 Tuning Knob
...
16. **GIVEN** LifecycleService 源文件，**WHEN** CI 测试扫描 import / preload，**THEN** **仅**对 `PersistenceService`、`InputService`、`AudioSystem` 三个 Foundation Autoload 有引用，对游戏层（Mochi Character、Text Input 等）**零**引用。

17. **GIVEN** LifecycleService 源文件，**WHEN** CI grep `OS\.get_ticks_msec`，**THEN** 零匹配（Godot 4.4+ 已弃用，必须使用 `Time.get_ticks_msec()`）。

18. **GIVEN** LifecycleService 源文件，**WHEN** CI grep `RESUME_DEBOUNCE_MS` / `BOOT_TIMEOUT_MS` 的裸字面量（`1000` / `5000`）出现在非字符串上下文，**THEN** 零匹配——所有 Tuning Knob 值读取自 `res://config/lifecycle_config.tres`（与 Persistence、Input 配置资源约定一致）。
...
| 问题 | 决议 |
|------|------|
| ~~Lifecycle 是否作为 OS 通知统一网关？~~ | **否**。三大 Foundation 同侪（Persistence/Input/Audio）按各自 GDD 设计直接订阅 OS 通知。Lifecycle 是上层协调服务，不做中介。 |
...
| ~~冷启动 "ready" 判定标准？~~ | Foundation 三同侪全部 `_ready()` 完成（Persistence ∈ READY_*、Input == GESTURE_MODE、Audio.is_ready() == true）。 |
...
| **AudioSystem 需要回填 `is_ready()` API**（Lifecycle 依赖此方法） | audio-director + `/consistency-check` | Lifecycle 进入实现前 | Dependencies 节已记录。建议作为 Audio Core Rule 6 的实现细节暴露。 |
| **PersistenceService 需要回填 `get_state()` API**（或等价访问器） | game-designer + `/consistency-check` | Lifecycle 进入实现前 | Dependencies 节已记录。Persistence 描述了 5 个状态但未暴露查询接口。 |
...
| Persistence `state` 字段（待回填 `get_state()` API） | `design/gdd/persistence-system.md` | States 表 | 接口待补充 |
...
| Audio `is_ready()` API（待 Audio GDD 回填） | `design/gdd/audio-system.md` | Core Rule 6 | 接口待补充 |
