...
1. **单例服务**：Input System 以 Autoload 单例 `InputService` 运行于 Foundation 层，在 Autoload 列表中位于 `PersistenceService` 之后（紧随其后）。它是全局的触摸手势识别器和模式控制器。
...
7. **应用后台中断**：当 `NOTIFICATION_APPLICATION_PAUSED` 触发时，Input System 强制取消所有进行中的手势（若 drag 进行中，发出 `drag_ended` 并清空追踪状态；若 long-press 计时中，取消计时）。恢复前台时，重置为 `GESTURE_MODE`（若当前为 TEXT_MODE，TextInput System 需重新调用 `request_text_mode()`）。
...
func request_text_mode() -> void      # TextInput 系统调用，IME 将弹出时
func release_text_mode() -> void      # TextInput 系统调用，IME 已收起时
...
| **Mobile App Lifecycle** | 无直接依赖 | Input System 直接订阅 `NOTIFICATION_APPLICATION_PAUSED`（与 Persistence 相同模式），不通过 Lifecycle 中间层 | — |
...
| 系统 | 关系 |
|------|------|
| **Mobile App Lifecycle** (#5) | 无直接依赖。Input System 直接订阅 OS 通知，不通过 Lifecycle 中间层（与 Persistence 相同模式）。两者均为 Foundation 层，无相互依赖。 |
...
18. **GIVEN** InputService 源文件，**WHEN** CI grep 扫描 `TAP_DRAG_THRESHOLD_PX`、`LONG_PRESS_THRESHOLD_MS` 的裸字面量（`12`、`500`），**THEN** 零匹配——所有阈值读取自 `res://config/input_config.tres`（与 Persistence 的配置资源约定一致）。
...
| **long-press 的 MVP 实现策略**：游戏概念中 MVP 无 long-press 交互。是在 MVP 阶段完整实现（含 AC 3/4）以备未来扩展，还是仅保留信号定义但不实现计时逻辑？ | 用户（设计决策） | Wave 1 Input GDD 设计完成后 | 推荐：完整实现，成本极低（一个计时器），避免 Wave 4+ 改接口 |
| **前台恢复后 IME 状态同步**：应用从后台恢复时，iOS 可能已自动收起 IME。TextInput System 需要可靠方式检测 IME 当前状态（Godot 4.6 是否提供 IME 可见性查询 API？），以便决定是否调用 `release_text_mode()`。 | godot-specialist | TextInput GDD 设计时 | 若无可靠 API，可考虑 TextInput 在 `NOTIFICATION_APPLICATION_FOCUS_IN` 时无条件调用 `release_text_mode()` 作为保守策略 |
