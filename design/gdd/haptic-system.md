...
9. **生命周期感知 — 自订 OS 通知**（与 Persistence / Input / Audio 三 Foundation 同侪同模式）：Haptic 在 `_ready()` 中调用 `get_tree().root.connect("application_paused", _on_app_paused)` 和 `application_resumed`（Godot 4.6 `MainLoop` notifications），**不**通过 Mobile App Lifecycle 转发。后台期间所有 `play()` no-op（避免唤醒 Taptic Engine 浪费电）。这一架构决策的理由：Foundation 同侪间不应有跨节点的强耦合订阅链；每个 Foundation 自管自己的 OS 集成。Lifecycle GDD line 317 已声明此模式。
...
| Mobile App Lifecycle (#5) | **无直接关系**——Haptic 自订 OS 通知 `application_paused/resumed`（与 Persistence/Input/Audio 同侪同模式）；Lifecycle GDD line 317 已确认此契约 | (无) |
