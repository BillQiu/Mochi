...
| 13a | **Juice Contract (Layer 1)** ⭐ | Meta | MVP | **In Design — pending /design-review** (2026-05-22 拆分路径首次落地；815 行；10 Core Rules + F-1 ±25ms 容差 + F-6 iOS reduce-motion 硬映射 + F-7 修 input range + F-strength 量化 Core Rule 10 + 14 AC 13/14 自动化；Pass 2 B13 WCAG / B14 reduce-motion MVP / B15 freeze 错路 / B16 Out-of-Scope / B17 audio 裂缝在 Layer 1 立法层处置) | design/gdd/juice-contract.md | Audio, Haptic, ADR-R (Renderer 待决), Lifecycle |
| 13b | **Juice Recipes (Layer 2)** ⭐ | Meta | MVP | **Not Started — 待 Layer 1 design review PASS 后启动** (7 个 JC-R 配方完整章节 + F-2/F-3/F-4/F-5 + VA-1..VA-7 + recipe AC) | design/gdd/juice-recipes.md（未创建）| Audio, Haptic, Layer 1 |
| 13c | ~~Game Feel / Juice Cookbook~~ (DEPRECATED) | Meta | MVP | **DEPRECATED — Layer 2 落地后转 stub 留 JC-R1..R7 ID 重定向** (Pass A commit ba160fd 内容保留为 Layer 2 起草输入；不再独立维护) | design/gdd/juice-cookbook.md | (历史，待 deprecation) |
