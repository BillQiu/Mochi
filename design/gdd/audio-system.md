...
2. **SFX 事件目录（MVP）**
   ...
   |--------|---------|------|---|-----------|
   | `lever_drag` | 摇杆开始拖拽（`drag_started`）→ 拖拽结束（`drag_ended` 或越阈值触发 `lever_pull`）| **是** | none（拖拽无触觉配对——连续音阶段离散冲击会与拖拽 fantasy 冲突） | Lever Interaction |
...
4. **API 接口（外部系统调用方式）**
   ...
   # play_loop / stop_loop 的合法键白名单（MVP）
   # 非白名单键调用 play_loop() 时降级为单次 play()，详见 AC-B-12
   var _valid_loop_keys: Dictionary[StringName, bool] = {
       &"shred_loop":  true,   # 粉碎过程持续音
       &"lever_drag":  true,   # 拖拽预紧连续音（per Lever Interaction MVP）
   }
   ```

5. **并发 SFX**：使用对象池（8 个 `AudioStreamPlayer` 节点）处理并发；**所有循环 SFX 各自使用独立的专用播放器节点，不参与池分配**——当前为 `shred_loop`（粉碎持续音）和 `lever_drag`（拖拽预紧音）两个专用节点，与 `_valid_loop_keys` 白名单一致。新增循环键时必须同步加一个专用节点。
...
8. **iOS Ring/Silent 开关**：遵守 iOS 物理静音开关——开关拨到静音时，BGM 和所有 SFX 均静音，与系统默认行为一致。Godot 在 iOS 上默认遵守此开关，无需额外代码。`play_loop()` 调用未在 `_valid_loop_keys` 白名单内的键时降级为单次 `play()`，打印警告日志（详见 AC-B-12）；MVP 中 `play_loop` 的合法键为 `&"shred_loop"` 与 `&"lever_drag"`（见 Core Rule 4 `_valid_loop_keys` 白名单）。

   **OS 通知订阅方式**（per ADR-0002 pattern）：AudioSystem 通过 `_notification(what)` 覆写方法直接响应 OS `NOTIFICATION_APPLICATION_PAUSED` 和 `NOTIFICATION_APPLICATION_RESUMED`，**不通过 LifecycleService 信号订阅**。这与 Persistence / Input / Haptic 的同侪模式一致——Foundation Autoloads 各自直接订阅 OS 通知，不通过 Lifecycle 转发。**注意**：早期 GDD 版本曾写 `NOTIFICATION_APPLICATION_FOCUS_IN`——那是窗口焦点通知（如桌面端 Alt+Tab），与移动端 lifecycle resume 语义不同。Godot 4.6 在 iOS/Android 上发出 `NOTIFICATION_APPLICATION_RESUMED` 表示前台恢复。
...
| `PAUSED` | App 已进入后台，所有音频挂起 | `NOTIFICATION_APPLICATION_PAUSED`（OS 直接通知） | `NOTIFICATION_APPLICATION_RESUMED`；恢复后回到 IDLE（shred_loop 已强制停止，不自动重启） |
...
| 调用方 | 调用的方法 | 触发时机 |
|--------|-----------|---------|
| Lever Interaction | `play_loop(&"lever_drag")` → `stop_loop(&"lever_drag")` → `play(&"lever_pull")` | `drag_started` 启动循环 → `drag_ended` 或越过触发阈值时停止循环；越阈值情形紧接发射 `lever_pull` 决定性一击（音触配对发射顺序见 Core Rule 9）|
...
| `FADE_DURATION` *(v1.0 only — MVP 不可调，per Core Rule 6 + F-3)* | 800ms | [400, 1500ms] | 淡出过长，用户离开上下文后还在淡出 | 淡出过短，听感像"切断" |
...
- **AC-F-07**（lever_drag 循环往返 + 不占池）：GIVEN `AudioSystem` 已初始化、`lever_drag` 资源已加载，WHEN 调用 `play_loop(&"lever_drag")`，THEN 专用 `lever_drag` 节点开始循环播放（`playing == true`、`stream.loop == true`）且 SFX 池空闲节点数量不减少；随后调用 `stop_loop(&"lever_drag")`，THEN 该专用节点停止（`playing == false`），且不影响 `shred_loop` 专用节点或池中其他 SFX。重复调用 `play_loop(&"lever_drag")` 不叠加第二个实例（对位 AC-B-02 的语义）。
...
- **AC-State-02**（PAUSED → IDLE 恢复）：GIVEN AudioSystem 在 PAUSED 状态，WHEN `NOTIFICATION_APPLICATION_RESUMED` 触发，THEN AudioSystem 返回 IDLE 状态；BGM 自动恢复（平台保证）；`shred_loop` 不自动重启（由 Shred Process 自主决定）。
