...
> **Status**: In Design
> **Author**: user (Bill Qiu) + game-designer + art-director + audio-director + technical-artist (lean mode — only D/H specialists active)
...
7. **Cookbook 配方的实现路径已锁定到 Godot 4.6 API**：`Tween` (scale/modulate/
   offset) · `CPUParticles2D` (≤32 粒子) · `Camera2D.offset` (shake) ·
   `AudioStreamPlayer2D` · `AnimationPlayer.speed_scale` (局部 freeze)。
   **禁止** Mobile renderer 下使用 Environment Glow / 多 pass shader /
   `Engine.time_scale`。详细 Forbidden 技术列表见每条配方下方 Forbidden 子项。
...
| **JC-R4** | `shred_pulse` · 让玩家感受到"机器还在工作，有节奏，不是等待" | particles (micro)：每脉冲追加 3-4 颗碎屑，方向偏腔体底部 + layered audio (micro)：粉碎 loop 音每 `shred_pulse_interval_s` (=0.4 s) 插入 +3 dB / 30 ms 的音量 bump | 40 ms / 脉冲，间隔 400 ms，共 3–5 次 | P2 |
...
- **JC-R3 Forbidden**：
  - *设计*：color flash 颜色禁止金色或高饱和彩色——这两种专属稀有产物揭晓 (v1.0+)；
    此处只用白/暖白亮度变化
  - *技术*：禁止 Environment Glow 模拟"内部光"——Mobile renderer 不支持；
    禁止 `CPUParticles2D` 粒子数 > 48（需为 JC-R4 叠加留余量）

- **JC-R4 Forbidden**：
  - *设计*：禁止同时触发 screen shake——规律化抖动会被误读为 bug 而非节奏；
    shake 的"意外性"是其力量来源
  - *技术*：禁止 shader uniform 动画（每帧修改 `ShaderMaterial` 参数）——Mobile renderer 下
    material rebind 累计成本不可预测，改用 `CanvasItem.modulate` Tween；禁止新增粒子 burst
...
- **JC-R6 Forbidden**：
  - *设计*：Pillar 4 重内容场景下禁止任何夸张 squash 或欢庆粒子——"轻重得当"的实现方式是
    **不区分**，而不是增加悼念感
  - *技术*：禁止填色 shader 多 pass（`render_mode blend_add`）——Mobile renderer overdraw 惩罚
...
| **Action** | Anticipation 结束 | 40–120 ms | 主体动作：拉杆按下、剪影上升、产物飞出 |
| **Impact** ⭐ | Action 到达视觉峰值 = Audio onset + 35 ms（`audio_lead_ms`）= Haptic strike ± 15 ms | 1–3 帧 | 视觉峰值帧、最大幅度——同步窗口的中心 |
| **Follow-through** | Impact 结束 | 60–180 ms | 回弹、衰减、稳定 |
| **Silence** | Follow-through 结束 | ≥ 100 ms（Core Rule 5） | 空白尾——下一个配方不允许在此期间发射 |

**关键约束**：Impact 相位的中心帧必须是 Audio onset 后 35 ms（`audio_lead_ms`）。这是 Cookbook 的同步契约。视觉若过早达到峰值（早于 Audio - 15 ms），表现为"先看到打击再听到声音"，玩家潜意识识别为"不真实"；若过晚（晚于 Audio + 50 ms），表现为"声音空响"。
...
| Onboarding (#15) | 复用 JC-R1-R7 的全套配方，但首个 product reveal 时 JC-R5 的 medium → **large** 临时升级（onboarding 唯一一次允许）；Onboarding GDD 必须显式声明此偏离 | 受控偏离 |
...
### F-1：同步对位公式（Impact 相位中心时刻）

`impact_center_t_ms = audio_onset_t_ms + audio_lead_ms`
= `audio_onset_t_ms + 35`
...
|---|---|---|---|---|
| `audio_onset_t_ms` | t_a | float | [0, ∞) | 当前帧 `AudioSystem.play(sfx_key)` 调用的实际起始时刻（毫秒，自 hook signal `emit()` 起计）|
| `audio_lead_ms` | — | int (registry) | =35（locked, haptic-system.md owns） | Haptic 相对 Audio 的延迟发射量 |
| `impact_center_t_ms` | t_i | float | [35, ∞) | 视觉 Impact 相位的中心帧时刻 |

**Output Range:** [35, ∞) ms（典型 hook 内 35-1500 ms）
**约束：** 视觉峰值帧时刻必须落在 `[t_i - 15, t_i + 15]` 窗口内（half of `audio_haptic_sync_window_ms = 30`）。
**Example:** `lever_lock` hook 在 t=0 调用 `AudioSystem.play("lever_pull")`，则 Impact 视觉峰值帧应落在 [20, 50] ms 之间，理想 35 ms。
**注意：** 此公式只规范视觉时序；Haptic 时序由调用方保证 `await timer(audio_lead_ms / 1000.0)` 后再 `Haptic.play()`，F-1 不控制触觉。
...
`active_count_r(t) = burst_count_r * ceil(overlap_factor_r)`
`overlap_factor_r = lifetime_ms_r / interval_ms_r`（interval = 0 时取 1，即单次 burst）

**约束（硬上限）：** `max over t (active_particles_total(t)) ≤ 48`（Mobile renderer iOS 安全上限，technical-artist 给出）

**Per-recipe budget table:**

| 配方 | burst_count | lifetime_ms | interval_ms | overlap_factor | max active 瞬时 |
|---|---|---|---|---|---|
| JC-R3 | 8-12 | 600 | (single burst) | 1 | 12 |
| JC-R4 | 3-4 / 脉冲 | 400 | 400 (= `shred_pulse_interval_s` × 1000) | ceil(400/400)=1, 但相邻批次刚好重叠 ⇒ 实际 2 | 8 |
| JC-R5 v1.0 (rare) | 16 | 400 | (single burst) | 1 | 16 |
| JC-R7 | 0 | — | — | 0 | 0 |
| **MVP 最坏（无 R5 rare）** | | | | | **20 同帧瞬时** |
| **v1.0 最坏（含 R5 rare）** | | | | | **36 同帧瞬时** ✅ < 48 |

**Output Range:** `active_particles_total ∈ [0, 48]`，MVP 实测上限约 20、v1.0 含 rare 约 36。
**注意：** F-4 是**最大瞬时**约束（≠ 时间积分），因为 GPU/CPU 粒子的性能瓶颈是单帧实例数。JC-R4 的"两批重叠"通过 `overlap_factor = ceil(lifetime/interval)` 显式建模。
...
| `anticipation_offset_px` | float | [0.1, 1.8] px | 反向预备的位移幅度 |

**Output Range:** [0.1, 1.8] px
**使用语义（避免量纲歧义）**：
...
| **Onboarding / First-Run System** (#15, Not Started) | Hard | 硬引用 + 受控偏离 | 全套 JC-R1-R7；唯一允许偏离：首产物 reveal 时 JC-R5 medium→large（s_overshoot 1.2→1.25 上限） | Onboarding GDD 必须显式声明"large 偏离"并附 first-run flag 状态机；Reduce Motion 优先级 > 偏离（Edge Case） |
...
**GIVEN** 下游 GDD 实现代码调用 `AudioSystem.play(sfx_key)` 并记录调用时刻 `t_a`，
**WHEN** 视觉 Impact 相位中心帧渲染时刻 `t_i` 被测量，
**THEN** `|t_i - (t_a + 35)| ≤ 15 ms` 对 JC-R1 至 JC-R7 所有配方均成立。

- **执行路径**：Manual real-device test（iOS 真机 + Xcode Instruments 帧时间戳；simulator 不可替代）
- **Gate Level**：BLOCKING
- **来源**：F-1 · Core Rule 3 · States and Transitions "Impact 相位中心帧约束"
- **备注**：Production Sprint 1 校准 `audio_lead_ms`；MVP 阶段配合视觉目测 + 帧计数 log 辅助。
...
- JC-R3：无金色/高饱和彩色 modulate、无 `Environment` Glow 节点、无 `CPUParticles2D.amount > 48`
- JC-R4：无 `Camera2D.offset` 写入、无 `ShaderMaterial` 参数每帧写入、无粒子 burst（只追加）
- JC-R5：无 `Camera2D.offset` 写入、无 `Engine.time_scale`
- JC-R6：无夸张 squash（s_compressed < 0.85）或粒子庆典、无多 pass blend_add shader
...
### AC-13：Pillar 4 重内容场景 JC-R6 振幅不变（反 attention-manipulation）

**GIVEN** Text Input GDD 标记当前输入为 heavy-content 模式（Pillar 4 激活），
**WHEN** 产物落地触发 `product_land` → JC-R6，
**THEN** `s_compressed = 0.92`、`s_overshoot = 1.0`、无额外粒子、无额外色彩变化——与非 heavy-content 场景**完全相同**；代码中无任何 Pillar 4 / heavy_content 分支改变 JC-R6 参数。

- **执行路径**：Code review check（实现文件无 heavy_content 条件分支）+ Manual real-device test（输入一段较重内容后观察产物落地动画与正常轻内容场景目测一致）
- **Gate Level**：ADVISORY
...
| Pillar 4 反 attention-manipulation | AC-13 | ADVISORY |

共 13 条 AC：8 BLOCKING / 5 ADVISORY。

### QA 执行路径分布

| 执行路径 | 适用 AC |
|---|---|
| Automated unit test (GUT) | AC-03, AC-04, AC-05, AC-06, AC-07 部分 |
| Manual real-device test | AC-01, AC-02, AC-07, AC-08, AC-09, AC-10, AC-13 |
| Code review check | AC-10, AC-11, AC-12 |
| Design review check | AC-12 |

## Open Questions

| ID | 问题 | 归属 | 目标解决期 |
|---|---|---|---|
| OQ-1 | `audio_lead_ms = 35 ms` 在真机实测后是否需要调整？目前是 audio_pipeline_latency_ms (40) - haptic_pipeline_latency_ms (5) 的中位数估计；扬声器 vs AirPods 路径会让 audio_pipeline_latency_ms 浮动 25-65 ms。Cookbook F-1 公式依赖此常量。 | Owner: technical-director；Source: haptic-system.md F-1 派生 | Production Sprint 1（真机校准） |
